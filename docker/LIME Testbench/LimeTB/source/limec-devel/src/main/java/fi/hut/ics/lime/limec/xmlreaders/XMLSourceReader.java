...
public class XMLSourceReader extends AbstractXMLReader {
	...
	private void parse() {
		...
		// scan for data structures (structs and unions) defined in this C 
		// source file
		...
		for (int i = 0; i < compoundNodes.getLength(); i++) {
			...
			for (int j = 0; j < compoundNode.getChildNodes().getLength(); j++) {
				...
				if (possibleInclude.getNodeName().equals("includes")) {
					// Found an #include statement. Associate it with this source file.
	...
	private void findDataStructures(Document doc) {
		...
		for(int i=0; i < innerclassTags.getLength(); i++) {
			...
			File structureXMLFile = new File(this.getXMLFile().getParent() + File.separator +
									fileName + ".xml");
			
			XMLDataStructureReader structureReader = 
				new XMLDataStructureReader(structureXMLFile, this.cSource); 
			
			this.cSource.addDataStructure(structureReader.getParsedSource());
	...
	private CSource createNewCSource(Document doc) {
		...
		NodeList compoundDefTags = doc.getDocumentElement().getElementsByTagName("compounddef");
		
		for (int i=0; i < compoundDefTags.getLength(); i++) {
			Node compoundDefTag = compoundDefTags.item(i);
			
			if (compoundDefTag.getAttributes().getNamedItem("kind").getNodeValue().equals("file")) {
	 ...
	 */
	public CSource getParsedSource() {
		return this.cSource;
	}
