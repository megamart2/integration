...
public class CMethodGenerator {
	...
	public static void createFunction(Node functionNode, CSource cSource) {
		...
		for (int j = 0; j < funcDetails.getLength(); j++) {
			...
			if (attributeNode.getNodeName().equals("type")) {
				// Store return type
				assert funcReturnType == null : "Double return type definition for function.";
				...
				CParameter cParameter = createCParameter(attributeNode);
		...
		CMethod tempFunction = new CMethod(cSource, funcName, 
		...
		for(Node funcISLNode : funcISLNodes) {
			ISLGenerator.createISL(funcISLNode, tempFunction);
		}
	 ...
	 */
	private static CParameter createCParameter(Node attributeNode) {
		...
		} else if (paramType.equals("struct") || paramType.equals("union")) {
		...
		CParameter tempParam = new CParameter(paramName, paramType, 
				paramIsArray);
