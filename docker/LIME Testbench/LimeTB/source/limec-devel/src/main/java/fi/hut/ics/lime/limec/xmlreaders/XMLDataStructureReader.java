...
public class XMLDataStructureReader extends AbstractXMLReader {
	...
	private void parseDataStructure(Document doc) {
		...
		// or an union).
		NodeList compounddefTag = doc.getDocumentElement().getElementsByTagName("compounddef");
		
		assert compounddefTag.getLength() == 1 : "Expected the XML file " +
				"containing a struct or union to have exactly one " + 
				"compounddef tag, but encountered " + compounddefTag.getLength() + ".";
		...
		NodeList compoundnameTag = doc.getDocumentElement().getElementsByTagName("compoundname");
		...
		if (structureType.equals("struct")) {
			...
			assert false : "Encountered a structure that was not a struct " +
						   "or union.";
		}
		
		NodeList memberdefTags = doc.getDocumentElement().getElementsByTagName("memberdef");
		...
		for(int i = 0; i < memberdefTags.getLength(); i++) {
			...
			if (memberdefType.getNodeValue().equals("variable")) {
				...
				// Add it accordingly.		 
				CVariable newVariable = CVariableGenerator.createVariable(memberdefTags.item(i), this.cSource);
				this.getParsedSource().addVariable(newVariable);
			}
		}
	 ...
	 */
	public CDataStructure getParsedSource() {
