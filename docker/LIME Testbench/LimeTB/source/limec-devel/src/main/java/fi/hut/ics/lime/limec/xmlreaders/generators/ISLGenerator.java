 ...
 * fi.hut.ics.lime.limec.islparser package.
 * 
 * @author lharpf
 */
...
import fi.hut.ics.lime.limec.islparser.ISLLexer;
import fi.hut.ics.lime.limec.islparser.ISLParser;
...
public class ISLGenerator {
	...
	public static void createISL(Node ISLNode, ISLContainer container) {
		...
		ISLParser parser = new ISLParser(new ISLLexer(new StringReader(ISLNode.getTextContent())));
		
		try {
			@SuppressWarnings("unchecked")
			LinkedList<Annotation> temp = (LinkedList<Annotation>)parser.parse().value;
