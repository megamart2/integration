...
public class DiffUtils {
    ...
    private static void processRawChunk(List<Object[]> rawChunk, Patch patch, int old_ln, int new_ln) {
    	String tag;
        String rest;
    	if (rawChunk.size() != 0) {
            ...
            if(oldChunkLines.isEmpty()) {
            	patch.addDelta(new InsertDelta(new Chunk(old_ln,oldChunkLines), new Chunk(new_ln-1,newChunkLines)));
            }
            else if(newChunkLines.isEmpty()) { 
            	patch.addDelta(new DeleteDelta(new Chunk(old_ln-1,oldChunkLines), new Chunk(new_ln, newChunkLines)));
        	}
        	else {
    ...
    public static List<UnifiedDiff> splitUnifiedDiff(List<String> lines) {
    	ArrayList<UnifiedDiff> unifieddiffs = new ArrayList<UnifiedDiff>();

		ArrayList<String> diff_specials = new ArrayList<String>();
		diff_specials.add("+");
		diff_specials.add("-");
		diff_specials.add("@");
		diff_specials.add(" ");
		...
		UnifiedDiff last_diff = null;
		boolean read_orig_filename = false;
		boolean read_revised_filename = false;
		int i = 1;
		for (String line : lines) {
			
			if(read_revised_filename) {
				String header_parts[] = line.split("\\s+");
	
				last_diff.setRevisedFilename(header_parts[1]);
				read_revised_filename = false;
			}
			
			if(read_orig_filename) {
				String header_parts[] = line.split("\\s+");
				last_diff.setOriginalFilename(header_parts[1]);
				read_orig_filename = false;
				read_revised_filename = true;
			}
			
			
			if (!diff_specials.contains(line.substring(0, 1))) {
				
				
				last_diff = new UnifiedDiff();
				unifieddiffs.add(last_diff);
				read_orig_filename = true;
			}
			
			
			
			last_diff.addLine(line);
			i++;
		}
